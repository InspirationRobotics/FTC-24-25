...
public final class ThreeDeadWheelLocalizer implements Localizer {
    public static class Params {
        public double leftYTicks = 0.0; // y position of the first parallel encoder (in tick units)
        public double rightYTicks = 1.0; // y position of the second parallel encoder (in tick units)
        public double centerXTicks = 0.0; // x position of the perpendicular encoder (in tick units)
    }
    ...
    public ThreeDeadWheelLocalizer(HardwareMap hardwareMap, double inPerTick, Pose2d pose) {
        ...
        left = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "left")));
        right = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "right")));
        center = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "center")));
